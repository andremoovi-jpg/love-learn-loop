...
-- 1. Drop existing problematic policies first
DROP POLICY IF EXISTS "Admins view profiles limited" ON public.profiles;
DROP POLICY IF EXISTS "Admins can update all profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins can delete profiles" ON public.profiles;
DROP POLICY IF EXISTS "Admins can view profiles with verification" ON public.profiles;
DROP POLICY IF EXISTS "Admins can update profiles with verification" ON public.profiles;
DROP POLICY IF EXISTS "Admins can delete profiles with verification" ON public.profiles;

-- 2. Create stricter admin policies with proper authentication verification
CREATE POLICY "admin_view_profiles_verified" 
ON public.profiles 
FOR SELECT 
USING (
...
CREATE POLICY "admin_update_profiles_verified" 
ON public.profiles 
FOR UPDATE 
USING (
...
CREATE POLICY "admin_delete_profiles_verified" 
ON public.profiles 
FOR DELETE 
USING (
  is_admin_user() 
...
-- 3. Create a public_profiles view that only exposes non-sensitive data
...
-- 4. Create a secure function for admin access to sensitive profile data
