...
-- Criar função corrigida com referências completas
...
AS $$
BEGIN
  -- Verificar se é admin (corrigido com alias completo)
  IF NOT EXISTS (
  ...
  -- Retornar dados combinados (corrigido com aliases completos)
  RETURN QUERY
  SELECT
    p.id,
    p.user_id,
